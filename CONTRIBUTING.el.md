# Συνεισφορά στο TopClaw

Σας ευχαριστούμε για το ενδιαφέρον σας να συνεισφέρετε στο TopClaw! Αυτός ο οδηγός θα σας βοηθήσει να ξεκινήσετε.
...
1. **Βρείτε ένα ζήτημα.** Αναζητήστε ζητήματα με την ετικέτα [`good first issue`](https://github.com/topway-ai/topclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — αυτά είναι σχεδιασμένα για νεοεισερχόμενους και περιλαμβάνουν το απαραίτητο πλαίσιο για να ξεκινήσετε γρήγορα.
...
4. **Ξεκινήστε με το Track A.** Το TopClaw χρησιμοποιεί τρία επίπεδα συνεργασίας (A/B/C) βάσει κινδύνου. Οι συνεισφέροντες για πρώτη φορά θα πρέπει να στοχεύουν στο **Track A** (τεκμηρίωση, δοκιμές, μικροεργασίες) — αυτά απαιτούν ελαφρύτερη αναθεώρηση και είναι η ταχύτερη διαδρομή για την ενσωμάτωση (merge) ενός PR.
...
Το TopClaw υποστηρίζει κλιμακωτή διαχείριση μυστικών για την τοπική ανάπτυξη και την υγιεινή του CI.
...
2. **Αρχείο ρυθμίσεων** (`~/.topclaw/config.toml`)
...
3. Γενικές μεταβλητές περιβάλλοντος (`TOPCLAW_API_KEY`, `API_KEY`)
...
**TopClaw** — Μηδενική επιβάρυνση. Κανένας συμβιβασμός. 🦀
