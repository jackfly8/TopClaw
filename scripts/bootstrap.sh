...
Options:
  ...
  --docker-reset             Reset existing TopClaw Docker containers/networks/volumes and data dir before --docker bootstrap
...
Environment:
  ...
  TOPCLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  TOPCLAW_DOCKER_IMAGE      Docker image tag to build/run (default: topclaw-bootstrap:local)
  TOPCLAW_DOCKER_BROWSER_RUNTIME
                            Browser runtime provisioning mode for --docker: "auto" (prompt), "on", or "off"
  TOPCLAW_DOCKER_BROWSER_SIDECAR_IMAGE
                            Browser WebDriver sidecar image (default: selenium/standalone-chromium:latest)
  TOPCLAW_DOCKER_BROWSER_SIDECAR_NAME
                            Browser WebDriver sidecar container name (default: topclaw-browser-webdriver)
  TOPCLAW_DOCKER_NETWORK    Docker network for TopClaw + sidecars (default: topclaw-bootstrap-net)
  TOPCLAW_DOCKER_CARGO_FEATURES
                            Extra Cargo features for Docker builds (comma-separated)
  TOPCLAW_CARGO_FEATURES    Extra Cargo features for local source builds (comma-separated)
  TOPCLAW_CONFIG_PATH       Config path used for channel feature auto-detection (default: ~/.topclaw/config.toml)
  TOPCLAW_DOCKER_DAEMON_NAME
                            Daemon container name for --docker-daemon (default: topclaw-daemon)
  TOPCLAW_DOCKER_DAEMON_BIND_HOST
                            Host bind address for daemon port publish (default: 127.0.0.1)
  TOPCLAW_DOCKER_DAEMON_HOST_PORT
                            Host port to publish daemon gateway (default: same as gateway.port)
  TOPCLAW_DOCKER_SECRET_KEY_FILE
                            Host path to .secret_key used when seeding encrypted config.toml
  TOPCLAW_API_KEY           Used when --api-key is not provided
  TOPCLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  TOPCLAW_MODEL             Used when --model is not provided
  TOPCLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  TOPCLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  TOPCLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${TOPCLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${TOPCLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/topway-ai/topclaw/releases/latest/download/topclaw-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/topclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name topclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable topclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/topclaw"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/topclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/topclaw-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  if prompt_yes_no "Install topclaw into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${TOPCLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set TOPCLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set TOPCLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
is_topclaw_container() {
  ...
  [[ "$name_lc" == *"topclaw"* || "$image_lc" == *"topclaw"* || "$command_lc" == *"topclaw"* ]]
}

is_topclaw_resource_name() {
  local name="$1"
  local name_lc
  name_lc="$(printf '%s' "$name" | tr '[:upper:]' '[:lower:]')"
  [[ "$name_lc" == *"topclaw"* ]]
...
maybe_stop_running_topclaw_containers() {
  ...
  while IFS=$'\t' read -r id name image command; do
    ...
    if is_topclaw_container "$name" "$image" "$command"; then
      running_ids+=("$id")
  ...
  warn "Detected running TopClaw container(s):"
  ...
  if ! guided_input_stream >/dev/null 2>&1; then
    warn "Non-interactive mode detected; leaving existing TopClaw containers running."
    return 0
  fi

  if prompt_yes_no "Stop these running TopClaw containers before continuing?" "yes"; then
    info "Stopping ${#running_ids[@]} TopClaw container(s)"
    "$CONTAINER_CLI" stop "${running_ids[@]}" >/dev/null
  else
    warn "Continuing with existing TopClaw containers still running."
  fi
}

reset_docker_topclaw_resources() {
  ...
  info "Resetting TopClaw Docker resources"
  ...
  if [[ ${#container_ids[@]} -gt 0 ]]; then
    info "Removing ${#container_ids[@]} TopClaw container(s)"
    ...
    info "No existing TopClaw containers found"
  fi
  ...
  if [[ ${#network_names[@]} -gt 0 ]]; then
    ...
    info "No existing TopClaw networks found"
  fi
  ...
  if [[ ${#volume_names[@]} -gt 0 ]]; then
    info "Removing ${#volume_names[@]} TopClaw volume(s)"
    ...
    info "No existing TopClaw volumes found"
  fi
...
run_docker_bootstrap() {
  ...
  fallback_image="ghcr.io/topway-ai/topclaw:latest"
  ...
  docker_browser_sidecar_image="${TOPCLAW_DOCKER_BROWSER_SIDECAR_IMAGE:-selenium/standalone-chromium:latest}"
  docker_network="${TOPCLAW_DOCKER_NETWORK:-topclaw-bootstrap-net}"
  docker_daemon_name="${TOPCLAW_DOCKER_DAEMON_NAME:-topclaw-daemon}"
  docker_daemon_bind_host="${TOPCLAW_DOCKER_DAEMON_BIND_HOST:-127.0.0.1}"
  docker_daemon_host_port="${TOPCLAW_DOCKER_DAEMON_HOST_PORT:-}"
  seed_config_path="${DOCKER_CONFIG_FILE:-}"
  seed_secret_key_path="${DOCKER_SECRET_KEY_FILE:-${TOPCLAW_DOCKER_SECRET_KEY_FILE:-}}"
  container_network_name=""
  docker_browser_webdriver_url=""
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.topclaw-docker"
  else
    default_data_dir="$WORK_DIR/.topclaw-docker"
  fi
  docker_data_dir="${TOPCLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  if [[ "$DOCKER_RESET" == true ]]; then
    reset_docker_topclaw_resources "$docker_data_dir"
  fi

  mkdir -p "$docker_data_dir/.topclaw" "$docker_data_dir/workspace"
  ...
  if [[ -n "$seed_config_path" ]]; then
    ...
    install -m 600 "$seed_config_path" "$docker_data_dir/.topclaw/config.toml"
    seed_docker_secret_key_for_config "$seed_config_path" "$docker_data_dir/.topclaw" "$seed_secret_key_path"
  fi
  ...
  case "$(printf '%s' "$docker_browser_runtime_mode" | tr '[:upper:]' '[:lower:]')" in
    ...
    *)
      ...
      if [[ "$docker_browser_runtime_bool" == "invalid" ]]; then
        warn "Invalid TOPCLAW_DOCKER_BROWSER_RUNTIME='$docker_browser_runtime_mode' (expected auto/on/off). Defaulting to off."
        docker_browser_runtime_bool="false"
      fi
      ;;
  esac
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if [[ -n "$docker_build_features" ]]; then
      ...
      docker_build_args+=(--build-arg "TOPCLAW_CARGO_FEATURES=$docker_build_features")
    fi
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official TopClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.topclaw:/topclaw-data/.topclaw"
  workspace_mount="$docker_data_dir/workspace:/topclaw-data/workspace"
  ...
  if [[ -n "$docker_browser_webdriver_url" ]]; then
    container_extra_env_args+=(-e "TOPCLAW_DOCKER_WEBDRIVER_URL=$docker_browser_webdriver_url")
  fi
  ...
  if [[ "$DOCKER_DAEMON_MODE" == true ]]; then
    ...
    if [[ -f "$docker_data_dir/.topclaw/config.toml" ]]; then
      config_gateway_port_value="$(config_gateway_port "$docker_data_dir/.topclaw/config.toml" || true)"
    fi
...
REPO_URL="https://github.com/topway-ai/topclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${TOPCLAW_CONTAINER_CLI:-docker}"
API_KEY="${TOPCLAW_API_KEY:-}"
PROVIDER="${TOPCLAW_PROVIDER:-openrouter}"
MODEL="${TOPCLAW_MODEL:-}"
LOCAL_CARGO_FEATURES="${TOPCLAW_CARGO_FEATURES:-}"
LOCAL_CONFIG_PATH="${TOPCLAW_CONFIG_PATH:-$HOME/.topclaw/config.toml}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${TOPCLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set TOPCLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  if [[ -n "$LOCAL_CARGO_FEATURES" ]]; then
    warn "--cargo-features / TOPCLAW_CARGO_FEATURES are ignored with --docker (use TOPCLAW_DOCKER_CARGO_FEATURES)."
  fi
  ensure_docker_ready
  ...
  echo
  echo "Your containerized TopClaw data is persisted under:"
  echo "  $DOCKER_DATA_DIR"
  echo

  if [[ "$DOCKER_DAEMON_MODE" == true ]]; then
    daemon_name="${TOPCLAW_DOCKER_DAEMON_NAME:-topclaw-daemon}"
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing topclaw to cargo bin"
...
TOPCLAW_BIN=""
if have_cmd topclaw; then
  TOPCLAW_BIN="topclaw"
elif [[ -x "$HOME/.cargo/bin/topclaw" ]]; then
  TOPCLAW_BIN="$HOME/.cargo/bin/topclaw"
elif [[ -x "$WORK_DIR/target/release/topclaw" ]]; then
  TOPCLAW_BIN="$WORK_DIR/target/release/topclaw"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$TOPCLAW_BIN" ]]; then
    error "onboarding requested but topclaw binary is not available."
    error "Run without --skip-install, or ensure topclaw is in PATH."
    exit 1
  fi
