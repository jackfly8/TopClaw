# TopClaw Config Reference (Operator-Oriented)
...
1. `TOPCLAW_WORKSPACE` override (if set)
2. persisted `~/.topclaw/active_workspace.toml` marker (if present)
3. default `~/.topclaw/config.toml`

TopClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `topclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `topclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `topclaw doctor traces --limit 20`
  - `topclaw doctor traces --event tool_call_result --contains \"error\"`
  - `topclaw doctor traces --id <trace-id>`
...
| `gated_actions` | `["file_write","browser_open","browser","memory_forget"]` | Tool actions protected by OTP |
...
- When `enabled = true` and no OTP secret exists, TopClaw generates one and prints an enrollment URI once.
- Gated tools and gated domains fail closed unless the tool call includes a valid `otp_code`.
...
| `state_file` | `~/.topclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `topclaw estop` to engage and `topclaw estop resume` to clear levels.
...
| Key | Default | Purpose |
|---|---|---|
| `format` | `bootstrap` | Identity format: `"bootstrap"` (default, markdown files) or `"aieos"` (JSON) |
...
3. Validate with `topclaw doctor` before restart/rollout.
...
Inter-process communication for independent TopClaw agents on the same host.

| Key | Default | Purpose |
|---|---|---|
| `enabled` | `false` | Enable IPC tools (`agents_list`, `agents_send`, `agents_inbox`, `state_get`, `state_set`) |
| `db_path` | `~/.topclaw/agents.db` | Shared SQLite database path (all agents on this host share one file) |
