...
- **Quota Tracking**: `topclaw providers-quota --provider qwen-code` shows static quota info (`?/1000` - unknown remaining, 1000/day total)
...
- After multimodal normalization, TopClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, TopClaw returns a structured capability error instead of silently ignoring images.
...
- TopClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `topclaw models refresh --provider llamacpp`
...
- Model discovery: `topclaw models refresh --provider sglang`
...
- Model discovery: `topclaw models refresh --provider vllm`
...
- Model discovery: `topclaw models refresh --provider osaurus`
...
Since TopClaw cannot auto-detect this, you can override it in `config.toml`:
...
Environment override: `TOPCLAW_MODEL_SUPPORT_VISION=true`
...
- Model discovery: `topclaw models refresh --provider nvidia`
