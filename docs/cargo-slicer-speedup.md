...
The CI workflow [`.github/workflows/ci-run.yml`](../.github/workflows/ci-run.yml) runs an accelerated release build alongside the standard one. It triggers on Rust-code changes and workflow changes, does not gate merges, and runs in parallel as a non-blocking check.
...
# Build with syn pre-analysis (from topclaw root)
