# TopClaw on Arduino Uno Q — Step-by-Step Guide

Run TopClaw on the Arduino Uno Q's Linux side. Telegram works over WiFi; GPIO control uses the Bridge (requires a minimal App Lab app).

---

## What's Included (No Code Changes Needed)

TopClaw includes everything needed for Arduino Uno Q. **Clone the repo and follow this guide — no patches or custom code required.**
...
## Phase 2: Install TopClaw on Uno Q
...
**At this point:** Telegram chat works. Send messages to your bot — TopClaw responds. No GPIO yet.

---

## Phase 5: GPIO via Bridge (TopClaw Handles It)

TopClaw includes the Bridge app and setup command.
...
Now when you message your Telegram bot *"Turn on the LED"* or *"Set pin 13 high"*, TopClaw uses `gpio_write` via the Bridge.
