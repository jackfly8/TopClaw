# Triển khai mạng — TopClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai TopClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | TopClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | TopClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — TopClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. TopClaw trên Raspberry Pi
...
- Channel Telegram hoạt động: TopClaw poll Telegram API (outbound)
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `topclaw tunnel` (xem tài liệu tunnel).

3. TopClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- TopClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `topclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `topclaw daemon`).
