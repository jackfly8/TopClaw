# TopClaw trên Arduino Uno Q — Hướng dẫn từng bước

Chạy TopClaw trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

TopClaw bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**

| Thành phần | Vị trí | Mục đích |
|------------|--------|---------|
| Bridge app | `firmware/topclaw-uno-q-bridge/` | MCU sketch + Python socket server (port 9999) cho GPIO |
...
## Phase 2: Cài đặt TopClaw trên Uno Q
...
ssh arduino@<UNO_Q_IP> "sudo mv ~/topclaw /usr/local/bin/"
```
...
## Phase 5: GPIO qua Bridge (TopClaw xử lý tự động)

TopClaw bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo topclaw):
```bash
...
Lệnh này copy Bridge app vào `~/ArduinoApps/topclaw-uno-q-bridge` và khởi động nó.
