...
1. Biến `TOPCLAW_WORKSPACE` (nếu được đặt)
2. Marker `~/.topclaw/active_workspace.toml` (nếu có)
3. Mặc định `~/.topclaw/config.toml`

TopClaw ghi log đường dẫn config đã giải quyết khi khởi động ở mức `INFO`:

- `Config loaded` với các trường: `path`, `workspace`, `source`, `initialized`

Lệnh xuất schema:

- `topclaw config schema` (xuất JSON Schema draft 2020-12 ra stdout)
...
- Biến môi trường: `TOPCLAW_MODEL_SUPPORT_VISION` hoặc `MODEL_SUPPORT_VISION` (giá trị: `true`/`false`/`1`/`0`/`yes`/`no`/`on`/`off`).
...
| `otel_service_name` | `topclaw` | Tên dịch vụ gửi đến OTLP collector |
...
1. `TOPCLAW_PROVIDER` (ghi đè tường minh, luôn thắng khi có giá trị)
...
- Dùng `TOPCLAW_PROVIDER` khi cố ý muốn biến môi trường ghi đè provider đã cấu hình.
...
- Khi đặt, ghi đè `TOPCLAW_CODEX_REASONING_EFFORT` cho OpenAI Codex.
- Để trống sẽ dùng `TOPCLAW_CODEX_REASONING_EFFORT` nếu có, nếu không mặc định `xhigh`.
...
- Mặc định an toàn: TopClaw **không** clone hay đồng bộ `open-skills` trừ khi `open_skills_enabled = true`.
- Ghi đè qua biến môi trường:
  - `TOPCLAW_OPEN_SKILLS_ENABLED` chấp nhận `1/0`, `true/false`, `yes/no`, `on/off`.
  - `TOPCLAW_OPEN_SKILLS_DIR` ghi đè đường dẫn kho khi có giá trị.
- Thứ tự ưu tiên: `TOPCLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` trong `config.toml` → mặc định `false`.
...
- TopClaw yêu cầu Composio v3 tools với `toolkit_versions=latest` và thực thi với `version="latest"` để tránh bản tool mặc định cũ.
...
3. Kiểm tra bằng `topclaw doctor` trước khi khởi động lại/triển khai.
...
- Khi `topclaw channel start` đang chạy, thay đổi `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url` và `reliability.*` được áp dụng nóng từ `config.toml` ở tin nhắn tiếp theo.
