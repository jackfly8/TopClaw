...
[INFO] ✓ Audit logging enabled (~/.config/topclaw/audit.log)
...
$ topclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/topclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ topclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ topclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ topclaw config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `topclaw security --status`
