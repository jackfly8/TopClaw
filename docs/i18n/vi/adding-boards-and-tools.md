# Thêm Board và Tool — Hướng dẫn phần cứng TopClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào TopClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.topclaw/config.toml)
...
Với feature `rag-pdf`, TopClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `topclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `topclaw peripheral list` | Liệt kê các board đã cấu hình |
| `topclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `topclaw peripheral flash` | Nạp firmware Arduino |
| `topclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `topclaw hardware discover` | Liệt kê thiết bị USB |
| `topclaw hardware info` | Thông tin chip qua probe-rs |
