# TopClaw trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy TopClaw trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

TopClaw có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
TopClaw bao gồm mọi thứ cần thiết cho Nucleo-F401RE:
...
| Flash command | `topclaw peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
Từ thư mục gốc của repo topclaw:

```bash
topclaw peripheral flash-nucleo
```

Lệnh này build `firmware/topclaw-nucleo` và chạy `probe-rs run --chip STM32F401RETx`. Firmware chạy ngay sau khi nạp xong.

### 1.3 Nạp thủ công (Phương án thay thế)

```bash
cd firmware/topclaw-nucleo
cargo build --release --target thumbv7em-none-eabihf
probe-rs run --chip STM32F401RETx target/thumbv7em-none-eabihf/release/topclaw-nucleo
```
...
## Phase 3: Cấu hình TopClaw

Thêm vào `~/.topclaw/config.toml`:
...
```bash
topclaw agent --message "Turn on the LED on pin 13"
```
...
| 5 | `topclaw daemon` hoặc `topclaw agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `topclaw peripheral list` để xác nhận.
