# Οδηγός Εγκατάστασης TopClaw σε Arduino Uno Q

Αυτός ο οδηγός περιγράφει τη διαδικασία εγκατάστασης και ρύθμισης του TopClaw στην πλευρά Linux του Arduino Uno Q.

## Επισκόπηση

Το TopClaw παρέχει πλήρη υποστήριξη για το Arduino Uno Q χωρίς να απαιτούνται αλλαγές στον κώδικα.

| Στοιχείο | Τοποθεσία | Περιγραφή |
|:---|:---|:---|
| Εφαρμογή Bridge | `firmware/topclaw-uno-q-bridge/` | MCU sketch και Python socket server για τη διαχείριση των GPIO. |
| Εργαλεία Bridge | `src/peripherals/uno_q_bridge.rs` | Εργαλεία `gpio_read` / `gpio_write` για επικοινωνία μέσω TCP. |
| Εντολή Setup | `src/peripherals/uno_q_setup.rs` | Η εντολή `topclaw peripheral setup-uno-q` για την ανάπτυξη του Bridge. |
...
## Βήμα 2: Εγκατάσταση του TopClaw
...
Βεβαιωθείτε ότι το αρχείο `~/.topclaw/config.toml` περιλαμβάνει τις απαραίτητες ρυθμίσεις για το Telegram και τον πράκτορα.

---

## Βήμα 4: Εκτέλεση του TopClaw Daemon

Ξεκινήστε την υπηρεσία TopClaw:
```bash
...
```bash
topclaw peripheral setup-uno-q --host <UNO_Q_IP>
```
