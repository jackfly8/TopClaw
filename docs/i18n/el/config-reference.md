# Οδηγός Ρυθμίσεων TopClaw (config.toml)
...
Το TopClaw ψάχνει για τις ρυθμίσεις με την εξής σειρά:
1. Στη διαδρομή που ορίζει η μεταβλητή `TOPCLAW_WORKSPACE`.
2. Στο αρχείο `~/.topclaw/config.toml` (αυτή είναι η συνηθισμένη θέση).
...
- Αρχεία πολιτικής/ταυτότητας όπως `AGENTS.md`, `SOUL.md`, `TOOLS.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md` και `BOOTSTRAP.md` θεωρούνται προστατευμένα και αλλάζουν χειροκίνητα.
- Αυτοματισμοί GUI / browser σε επίπεδο λειτουργικού μένουν απενεργοποιημένοι από προεπιλογή. Αποφύγετε `backend = "computer_use"` εκτός αν δουλεύετε σε απομονωμένο μηχάνημα.
...
- Αν αλλάξετε το αρχείο `config.toml`, πρέπει να κάνετε επανεκκίνηση το TopClaw για να δει τις αλλαγές.
- Χρησιμοποιήστε την εντολή `topclaw doctor` για να βεβαιωθείτε ότι οι ρυθμίσεις σας είναι σωστές.
