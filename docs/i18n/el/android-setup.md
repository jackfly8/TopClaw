...
Το TopClaw παρέχει προκατασκευασμένα εκτελέσιμα αρχεία (binaries) για συσκευές Android.
...
Ο ευκολότερος τρόπος εκτέλεσης του TopClaw σε Android είναι μέσω [Termux](https://termux.dev/).
...
Για προχωρημένους χρήστες που θέλουν να εκτελέσουν το TopClaw εκτός Termux:

```bash
# Από τον υπολογιστή σας με ADB
adb push topclaw /data/local/tmp/
adb shell chmod +x /data/local/tmp/topclaw
adb shell /data/local/tmp/topclaw --version
```
