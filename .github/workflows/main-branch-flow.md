...
1. External contributor opens PR from `fork/<branch>` into `topclaw:dev`.
...
5. In publish mode, workflow generates SBOM (`CycloneDX` + `SPDX`), `SHA256SUMS`, and a checksum provenance statement (`topclaw.sha256sums.intoto.json`) plus audit-event envelope.
