...
TopClaw uses GitHub's private vulnerability reporting and advisory workflow for important security issues.
...
TopClaw uses defense-in-depth controls.
...
TopClaw images follow CIS Docker Benchmark-oriented hardening.
...
docker build -t topclaw .
docker inspect --format='{{.Config.User}}' topclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace topclaw gateway
```
