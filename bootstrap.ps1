...
<#
.SYNOPSIS
  Windows bootstrap entrypoint for TopClaw.
...
function Ensure-RustToolchain {
    ...
    $tempDir = Join-Path $env:TEMP "topclaw-bootstrap-rustup"
...
function Install-PrebuiltBinary {
    ...
    try {
        ...
        $binary = Get-ChildItem -Path $extractDir -Recurse -Filter "topclaw.exe" | Select-Object -First 1
        if (-not $binary) {
            throw "Downloaded archive does not contain topclaw.exe"
        }

        $installDir = Join-Path $env:USERPROFILE ".cargo\bin"
        New-Item -ItemType Directory -Path $installDir -Force | Out-Null
        $dest = Join-Path $installDir "topclaw.exe"
...
function Resolve-TopClawBinary {
    $cargoBin = Join-Path $env:USERPROFILE ".cargo\bin\topclaw.exe"
    if (Test-Path $cargoBin) {
        return $cargoBin
    }

    $fromPath = Get-Command topclaw -ErrorAction SilentlyContinue
    if ($fromPath) {
...
function Run-Onboarding {
    ...
    if (-not $BinaryPath) {
        throw "Onboarding requested but topclaw binary is not available."
    }
    ...
    if (-not $resolvedApiKey) {
        $resolvedApiKey = $env:TOPCLAW_API_KEY
    }
...
$topclawBin = Resolve-TopClawBinary
if (-not $topclawBin) {
    throw "TopClaw binary was not found after bootstrap."
}

Write-Info "TopClaw bootstrap completed."
Write-Info "Binary: $topclawBin"

if ($Onboard) {
    Run-Onboarding -BinaryPath $topclawBin
}
