...
```bash
pip install topclaw-tools
```

With Discord integration:

```bash
pip install topclaw-tools[discord]
```
...
```python
import asyncio
from topclaw_tools import create_agent, shell, file_read, file_write
...
# Run the CLI
topclaw-tools "List files in the current directory"
...
```python
import os
from topclaw_tools.integrations import DiscordBot
...
```python
from topclaw_tools import tool
...
## Comparison with Rust TopClaw

| Feature | Rust TopClaw | topclaw-tools |
|---------|---------------|----------------|
...
Use **Rust TopClaw** for production edge deployments. Use **topclaw-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
