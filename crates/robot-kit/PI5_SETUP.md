...
Complete guide to setting up a TopClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.topclaw/models
cp models/ggml-base.bin ~/.topclaw/models/
```
...
mkdir -p ~/.topclaw/models/piper
cd ~/.topclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.topclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
# Build main topclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.topclaw
nano ~/.topclaw/robot.toml
```

```toml
# ~/.topclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.topclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/topclaw_sensors.fifo"
...
```bash
# Configure TopClaw to use robot tools
cat > ~/.topclaw/config.toml << 'EOF'
...
cp ~/topclaw/crates/robot-kit/SOUL.md ~/.topclaw/workspace/

# Start agent
./target/release/topclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/topclaw_sensors.fifo ]; then
    mkfifo /tmp/topclaw_sensors.fifo
fi
...
```bash
# /etc/systemd/system/topclaw-robot.service
sudo tee /etc/systemd/system/topclaw-robot.service << 'EOF'
[Unit]
Description=TopClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/topclaw
...
sudo systemctl enable topclaw-robot
sudo systemctl start topclaw-robot

# Check status
sudo systemctl status topclaw-robot
journalctl -u topclaw-robot -f  # View logs
```
