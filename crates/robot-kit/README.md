...
A complete toolkit for building AI-powered robots with TopClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build TopClaw with robot tools

```bash
# Clone and build
git clone https://github.com/topway-ai/topclaw
cd TopClaw
...
# Copy config
mkdir -p ~/.topclaw
cp crates/robot-kit/robot.toml ~/.topclaw/
cp crates/robot-kit/SOUL.md ~/.topclaw/workspace/
...
# Test in mock mode
./target/release/topclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/topclaw agent -m "Move forward 1 meter"
```
...
```rust
use topclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `topclaw`, add a thin adapter that maps this
...
mkdir topclaw-robot-kit
cp -r target/release/topclaw topclaw-robot-kit/
cp -r examples/robot_kit topclaw-robot-kit/
cp -r ~/.topclaw topclaw-robot-kit/dot-topclaw

# Include models
mkdir -p topclaw-robot-kit/models
cp ~/.topclaw/models/ggml-base.bin topclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf topclaw-robot-kit.tar.gz topclaw-robot-kit/

# Copy to USB
cp topclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
