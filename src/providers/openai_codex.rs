...
impl OpenAiCodexProvider {
    pub fn new(
        ...
        let state_dir = options
            .topclaw_dir
            .clone()
            .unwrap_or_else(default_topclaw_dir);
...
fn resolve_reasoning_effort(model_id: &str, override_level: Option<&str>) -> String {
    ...
    let env_level = std::env::var("TOPCLAW_CODEX_REASONING_EFFORT")
        .ok()
        .and_then(|value| {
            normalize_reasoning_level(Some(&value), "TOPCLAW_CODEX_REASONING_EFFORT")
        });
    ...
    async fn send_responses_request(
        ...
        let access_token = if use_gateway_api_key_auth {
            ...
            Some(oauth_access_token.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex auth profile not found. Run `topclaw auth login --provider openai-codex`."
                )
            })?)
        };
        let account_id = if use_gateway_api_key_auth {
            ...
            Some(account_id.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex account id not found in auth profile/token. Run `topclaw auth login --provider openai-codex` again."
                )
            })?)
        };
...
mod tests {
    ...
    fn resolve_reasoning_effort_prefers_config_override() {
        let _env_lock = env_lock();
        let _reasoning_guard = EnvGuard::set("TOPCLAW_CODEX_REASONING_EFFORT", Some("low"));
    ...
    fn resolve_reasoning_effort_falls_back_to_env_when_override_invalid() {
        let _env_lock = env_lock();
        let _reasoning_guard = EnvGuard::set("TOPCLAW_CODEX_REASONING_EFFORT", Some("medium"));
