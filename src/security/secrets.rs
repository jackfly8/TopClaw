...
// in `~/.topclaw/.secret_key` with restrictive file permissions (0600). The
...
//
// The only supported encrypted format is `enc2:` (ChaCha20-Poly1305).
...
pub struct SecretStore {
    /// Path to the key file (`~/.topclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(topclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: topclaw_dir.join(".secret_key"),
            enabled,
        }
    }
...
mod tests {
    ...
    fn decrypt_plaintext_passthrough() {
        let tmp = TempDir::new().unwrap();
        let store = SecretStore::new(tmp.path(), true);
        // Values without an encryption prefix are returned as-is.
