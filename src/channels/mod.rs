...
BTC is currently around $65,000 based on latest tool output."#
    ...
    #[tokio::test]
    async fn process_channel_message_rejects_advanced_provider_switches_in_channel_runtime() {
        let channel_impl = Arc::new(TelegramRecordingChannel::default());
        let channel: Arc<dyn Channel> = channel_impl.clone();
        ...
        assert_eq!(sent.len(), 1);
        assert!(
            sent[0].contains("Channel provider switching is limited to product-priority providers")
        );
        assert!(sent[0].contains("`anthropic` remains available through config.toml or CLI"));
        assert!(runtime_ctx
            .route_overrides
            .lock()
            .unwrap_or_else(|e| e.into_inner())
