...
pub(super) fn build_runtime_capability_inventory_prompt(
    tools_registry: &[Box<dyn Tool>],
    excluded_tools: &[String],
) -> String {
    let mut prompt = String::new();
    ...
    prompt.push_str(
        "This section is generated from current runtime policy for this message. \
         The tools below exist in the current runtime, but this specific turn must remain tool-free.\n\n",
    );

    prompt.push_str("- Tool execution in this turn: disabled by turn policy\n");
    if specs.is_empty() {
        prompt.push_str("- Installed runtime tools available outside this turn: (none)\n");
    } else {
        let _ = writeln!(
            prompt,
            "- Installed runtime tools available outside this turn ({}):",
            specs.len()
        );
        for spec in &specs {
            let _ = writeln!(prompt, "  - `{}`", spec.name);
    ...
    prompt.push_str(
        "- Do not describe the listed runtime tools as missing or permanently unavailable just because this turn is tool-free.\n",
    );
    prompt.push_str(
        "- If the user asks what you can do, distinguish between runtime capabilities, tools that may still require approval, and the fact that this turn should stay tool-free.\n",
    );
    prompt.push_str(
        "- No tool calls are allowed in this turn; answer from current context or ask one brief clarifying question.\n",
    );
