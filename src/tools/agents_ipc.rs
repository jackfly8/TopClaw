//! Inter-process communication tools for independent TopClaw agents.
//!
//! Provides 5 LLM-callable tools backed by a shared SQLite database, allowing
//! independent TopClaw processes on the same host to discover each other and
...
/// Shared SQLite handle for IPC tools. Each TopClaw process holds one instance.
...
mod tests {
    ...
    fn schema_creates_three_tables() {
        ...
        let db = test_db(&dir, "topclaw_agent_a");
    ...
    fn agent_registers_on_open() {
        ...
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM agents WHERE agent_id = 'topclaw_agent_a'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn heartbeat_updates_last_seen() {
        ...
        let before: i64 = {
            let conn = db.conn.lock().unwrap();
            conn.query_row(
                "SELECT last_seen FROM agents WHERE agent_id = 'topclaw_agent_a'",
                [],
                |row| row.get(0),
            )
            .unwrap()
    ...
    async fn inbox_isolates_per_agent() {
        ...
        let db_a = Arc::new(IpcDb::open_with_id(&db_path, "topclaw_agent_a", 300).unwrap());
        let db_b = Arc::new(IpcDb::open_with_id(&db_path, "topclaw_agent_b", 300).unwrap());

        // Agent A sends to Agent B
        let send_tool = AgentsSendTool::new(db_a.clone(), Arc::new(SecurityPolicy::default()));
        send_tool
            .execute(json!({"to_agent": "topclaw_agent_b", "payload": "hello b"}))
            .await
            .unwrap();
    ...
    async fn stale_agents_excluded_from_list() {
        ...
        let db_a = Arc::new(IpcDb::open_with_id(&db_path, "topclaw_agent_a", 5).unwrap());
        ...
        // Manually backdate agent B's last_seen
        {
            ...
            conn.execute(
                "INSERT OR REPLACE INTO agents (agent_id, status, last_seen) VALUES ('topclaw_agent_b', 'online', ?1)",
        ...
        assert_eq!(agents[0]["agent_id"], "topclaw_agent_a");
    }
    ...
    async fn identity_code_enforced() {
        let dir = TempDir::new().unwrap();
        let db_path = dir.path().join("agents.db").to_str().unwrap().to_string();
        let db = Arc::new(IpcDb::open_with_id(&db_path, "topclaw_agent_a", 300).unwrap());
        ...
        send_tool
            .execute(json!({"to_agent": "topclaw_agent_b", "payload": "test"}))
            .await
            .unwrap();
        ...
        let from: String = conn
            ...
            .unwrap();
        assert_eq!(from, "topclaw_agent_a");
    }

    #[tokio::test]
    async fn state_upsert_creates_and_updates() {
        let dir = TempDir::new().unwrap();
        let db = Arc::new(test_db(&dir, "topclaw_agent_a"));
    ...
    async fn state_records_owner() {
        ...
        let parsed: serde_json::Value = serde_json::from_str(&result.output).unwrap();
        assert_eq!(parsed["owner"], "topclaw_agent_a");
    }
    ...
    fn drop_removes_agent_from_table() {
        ...
        {
            let _db = IpcDb::open_with_id(&db_path_str, "topclaw_agent_a", 300).unwrap();
    ...
    async fn direct_messages_marked_read_after_inbox() {
        ...
        send_tool
            .execute(json!({"to_agent": "topclaw_agent_b", "payload": "once"}))
            .await
            .unwrap();
    ...
    async fn send_missing_params_returns_error() {
        ...
        // Missing payload
        let result = send_tool
            .execute(json!({"to_agent": "topclaw_agent_b"}))
            .await
            .unwrap();
        assert!(!result.success);
    ...
    async fn two_agents_full_exchange() {
        ...
        let r = send_a
            .execute(json!({"to_agent": "topclaw_agent_b", "payload": "task: summarize"}))
            .await
            .unwrap();
        assert!(r.success);
        ...
        assert_eq!(msgs[0]["from_agent"], "topclaw_agent_a");

        // Agent B replies to Agent A
        let send_b = AgentsSendTool::new(db_b.clone(), security.clone());
        send_b
            .execute(json!({"to_agent": "topclaw_agent_a", "payload": "done: summary attached"}))
            .await
            .unwrap();
        ...
        assert_eq!(msgs[0]["from_agent"], "topclaw_agent_b");
