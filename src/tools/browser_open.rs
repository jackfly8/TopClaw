    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                ...
                "otp_code": {
                    "type": "string",
                    "description": "One-time password required when browser_open or the target domain is OTP-gated by security policy"
                }
            },
            "required": ["url"]
        })
...
mod tests {
    ...
    async fn execute_blocks_gated_domain_without_otp() {
        ...
        let result = tool
            ...
            .unwrap();
        assert!(!result.success);
        assert!(result.error.unwrap().contains("OTP code required"));
    }
}
