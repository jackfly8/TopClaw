    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                ...
                "otp_code": {
                    "type": "string",
                    "description": "One-time password required when shell is OTP-gated by security policy"
                }
            },
            "required": ["command"]
        })
...
mod tests {
    ...
    fn test_security_with_otp(tmp: &TempDir) -> (Arc<SecurityPolicy>, String) {
        ...
        let security = Arc::new(SecurityPolicy {
            ...
            otp_gated_actions: ["shell".to_string()].into_iter().collect(),
    ...
    #[tokio::test]
    async fn shell_requires_otp_when_gated() {
        let tmp = tempfile::tempdir().unwrap();
        let (security, code) = test_security_with_otp(&tmp);
        let tool = ShellTool::new(security, test_runtime());

        let denied = tool
            .execute(json!({"command": "echo gated"}))
            .await
            .unwrap();
        ...
        assert!(allowed.output.contains("gated"));
    }
    ...
    async fn shell_does_not_leak_api_key() {
        ...
        let _g2 = EnvGuard::set("TOPCLAW_API_KEY", "sk-test-secret-67890");
        ...
        assert!(
            ...
            "TOPCLAW_API_KEY leaked to shell command output"
        );
    }
    ...
    async fn shell_allows_configured_env_passthrough() {
        let _guard = EnvGuard::set("TOPCLAW_TEST_PASSTHROUGH", "db://unit-test");
        let tool = ShellTool::new(
            test_security_with_env_passthrough(&["TOPCLAW_TEST_PASSTHROUGH"]),
            test_runtime(),
        );
        ...
        assert!(result
            .output
            .contains("TOPCLAW_TEST_PASSTHROUGH=db://unit-test"));
    }
    ...
    async fn shell_requires_approval_for_medium_risk_command() {
        ...
        let denied = tool
            .execute(json!({"command": "touch topclaw_shell_approval_test"}))
            .await
        ...
        let allowed = tool
            .execute(json!({
                "command": "touch topclaw_shell_approval_test",
                "approved": true
            }))
            .await
