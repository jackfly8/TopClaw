//! Performance benchmarks for TopClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/topway-ai/topclaw/issues/618 (item 7)
...
use topclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use topclaw::config::MemoryConfig;
use topclaw::memory;
use topclaw::memory::{Memory, MemoryCategory};
use topclaw::observability::{NoopObserver, Observer};
use topclaw::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
...
<tool_call>
{"name": "search", "arguments": {"query": "topclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "topclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about topclaw agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("topclaw agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
