...
```bash
# Check config
cat ~/.topclaw/config.toml | grep -A 5 telegram
...
RUST_LOG=debug topclaw channel start
...
| Binary size | ~3-4MB | `ls -lh target/release/topclaw` |
| Health check | <5s | `time topclaw channel doctor` |
| First response | <3s | Manual test in Telegram |
| Message split | <50ms | Check debug logs |
| Memory usage | <10MB | `ps aux \| grep topclaw` |
...
5. **Monitor logs** for any issues:
   ...
   topclaw daemon
   # or
   RUST_LOG=info topclaw channel start
   ```
...
- Issues: https://github.com/jackfly8/TopClaw/issues
