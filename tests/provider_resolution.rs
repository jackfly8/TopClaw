...
use topclaw::providers::compatible::{AuthStyle, OpenAiCompatibleProvider};
use topclaw::providers::{create_provider, create_provider_with_options, create_provider_with_url};
...
fn factory_resolves_openai_codex_provider() {
    let options = topclaw::providers::ProviderRuntimeOptions::default();
...
async fn model_fallback_within_same_provider() {
    ...
    use std::sync::Arc;
    use topclaw::providers::reliable::ReliableProvider;
    use topclaw::providers::traits::{ChatMessage, Provider};
    ...
    impl Provider for GeminiMock {
        ...
        async fn chat_with_tools(
            ...
            _model: &str,
            _temp: f64,
        ) -> anyhow::Result<topclaw::providers::traits::ChatResponse> {
            unimplemented!()
        }
    }
