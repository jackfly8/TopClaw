...
//!
//! Ref: https://github.com/topway-ai/topclaw/issues/618 (item 6)

use anyhow::Result;
use async_trait::async_trait;
use serde_json::json;
use std::sync::{Arc, Mutex};
use topclaw::agent::agent::Agent;
use topclaw::agent::dispatcher::{NativeToolDispatcher, XmlToolDispatcher};
use topclaw::agent::memory_loader::MemoryLoader;
use topclaw::config::MemoryConfig;
use topclaw::memory;
use topclaw::memory::Memory;
use topclaw::observability::{NoopObserver, Observer};
use topclaw::providers::traits::ChatMessage;
use topclaw::providers::{
...
async fn e2e_multi_turn_with_memory_enrichment() {
    ...
    assert!(req1_user.content.contains("project: topclaw"));
...
///
/// Requires valid OAuth credentials in `~/.topclaw/`.
...
///
/// Requires valid credentials in `~/.topclaw/`.
...
async fn e2e_live_research_phase() {
    use std::sync::Arc;
    use topclaw::agent::research::{run_research_phase, should_trigger};
    ...
    let observer: Arc<dyn topclaw::observability::Observer> = Arc::new(NoopObserver);
...
async fn e2e_agent_research_prompt_guided() {
    use topclaw::config::{ResearchPhaseConfig, ResearchTrigger};
    use topclaw::providers::traits::ProviderCapabilities;
